...
export function registerAllTools(server: McpServer): void {
  // Account Tools
  server.registerTool("create_account", CREATE_ACCOUNT, createAccountHandler);
  server.registerTool("decrypt_value", DECRYPT_VALUE, decryptValueHandler);
  server.registerTool("decrypt_message", DECRYPT_MESSAGE, decryptMessageHandler);
  server.registerTool("get_current_network", GET_CURRENT_NETWORK, getCurrentNetworkHandler);
  server.registerTool("get_current_rpc", GET_CURRENT_RPC, getCurrentRpcHandler);
  server.registerTool("encrypt_value", ENCRYPT_VALUE, encryptValueHandler);
  server.registerTool("encrypt_message", ENCRYPT_MESSAGE, encryptMessageHandler);
  server.registerTool("generate_aes_key", GENERATE_AES_KEY, generateAesKeyHandler);
  server.registerTool("import_account_from_private_key", IMPORT_ACCOUNT_FROM_PRIVATE_KEY, importAccountFromPrivateKeyHandler);
  server.registerTool("sign_message", SIGN_MESSAGE, signMessageHandler);
  server.registerTool("switch_network", SWITCH_NETWORK, switchNetworkHandler);
  server.registerTool("verify_signature", VERIFY_SIGNATURE, verifySignatureHandler);

  // Compiler & Deployment Tools
  server.registerTool("compile_contract", COMPILE_CONTRACT, compileContractHandler);
  server.registerTool("compile_and_deploy_contract", COMPILE_AND_DEPLOY_CONTRACT, compileAndDeployContractHandler);

  // ERC20 Tools
  server.registerTool("approve_erc20_spender", APPROVE_ERC20_SPENDER, approveERC20SpenderHandler);
  server.registerTool("deploy_private_erc20_contract", DEPLOY_PRIVATE_ERC20_CONTRACT, deployPrivateERC20ContractHandler);
  server.registerTool("get_private_erc20_allowance", GET_ERC20_ALLOWANCE, getERC20AllowanceHandler);
  server.registerTool("get_private_erc20_balance", GET_PRIVATE_ERC20_TOKEN_BALANCE, getPrivateERC20BalanceHandler);
  server.registerTool("get_private_erc20_decimals", GET_PRIVATE_ERC20_DECIMALS, getPrivateERC20DecimalsHandler);
  server.registerTool("get_private_erc20_total_supply", GET_PRIVATE_ERC20_TOTAL_SUPPLY, getPrivateERC20TotalSupplyHandler);
  server.registerTool("mint_private_erc20_token", MINT_PRIVATE_ERC20_TOKEN, mintPrivateERC20TokenHandler);
  server.registerTool("transfer_private_erc20", TRANSFER_PRIVATE_ERC20_TOKEN, transferPrivateERC20TokenHandler);

  // ERC721 Tools
  server.registerTool("approve_private_erc721", APPROVE_PRIVATE_ERC721, approvePrivateERC721Handler);
  server.registerTool("deploy_private_erc721_contract", DEPLOY_PRIVATE_ERC721_CONTRACT, deployPrivateERC721ContractHandler);
  server.registerTool("get_private_erc721_approved", GET_PRIVATE_ERC721_APPROVED, getPrivateERC721ApprovedHandler);
  server.registerTool("get_private_erc721_balance", GET_PRIVATE_ERC721_BALANCE, getPrivateERC721BalanceHandler);
  server.registerTool("get_private_erc721_is_approved_for_all", GET_PRIVATE_ERC721_IS_APPROVED_FOR_ALL, getPrivateERC721IsApprovedForAllHandler);
  server.registerTool("get_private_erc721_token_owner", GET_PRIVATE_ERC721_TOKEN_OWNER, getPrivateERC721TokenOwnerHandler);
  server.registerTool("get_private_erc721_token_uri", GET_PRIVATE_ERC721_TOKEN_URI, getPrivateERC721TokenURIHandler);
  server.registerTool("get_private_erc721_total_supply", GET_PRIVATE_ERC721_TOTAL_SUPPLY, getPrivateERC721TotalSupplyHandler);
  server.registerTool("mint_private_erc721_token", MINT_PRIVATE_ERC721_TOKEN, mintPrivateERC721TokenHandler);
  server.registerTool("set_private_erc721_approval_for_all", SET_PRIVATE_ERC721_APPROVAL_FOR_ALL, setPrivateERC721ApprovalForAllHandler);
  server.registerTool("transfer_private_erc721", TRANSFER_PRIVATE_ERC721_TOKEN, transferPrivateERC721TokenHandler);

  // Transaction Tools
  server.registerTool("call_contract_function", CALL_CONTRACT_FUNCTION, callContractFunctionHandler);
  server.registerTool("decode_event_data", DECODE_EVENT_DATA, decodeEventDataHandler);
  server.registerTool("get_transaction_status", GET_TRANSACTION_STATUS, getTransactionStatusHandler);
  server.registerTool("get_transaction_logs", GET_TRANSACTION_LOGS, getTransactionLogsHandler);

  // Private Message Tools
  server.registerTool("deploy_private_message_contract", DEPLOY_PRIVATE_MESSAGE_CONTRACT, deployPrivateMessageContractHandler);
  server.registerTool("send_private_message", SEND_PRIVATE_MESSAGE, sendPrivateMessageHandler);
  server.registerTool("read_private_message", READ_PRIVATE_MESSAGE, readPrivateMessageHandler);
  server.registerTool("get_private_message_count", GET_PRIVATE_MESSAGE_COUNT, getPrivateMessageCountHandler);
  server.registerTool("get_private_message_senders", GET_PRIVATE_MESSAGE_SENDERS, getPrivateMessageSendersHandler);

  // Native Tools
  server.registerTool("get_native_balance", GET_NATIVE_BALANCE, getNativeBalanceHandler);
  server.registerTool("transfer_native", TRANSFER_NATIVE, transferNativeHandler);
}
